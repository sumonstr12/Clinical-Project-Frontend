...
import { 
  Search, Loader2, ChevronLeft, ChevronRight, X, User, Mail, Phone, 
  ...
  AlertCircle, Pill, Syringe, Weight, Ruler, Droplet, MapPin 
} from "lucide-react";
...
// Appointment Profile Modal Component
          ...
          <>
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <div className="flex items-center gap-3 py-2 px-3 rounded-lg bg-white/3 border border-white/5">
                  <Avatar 
                    name={appointment.provider?.user?.full_name} 
                    imgUrl={appointment.provider?.img_url}
                  />
                  <div className="flex-1 min-w-0">
...
export default function AppointmentList() {
  ...
  const fetchAppointments = useCallback(async () => {
    setLoading(true);
    try {
      let query = `/admin/appointment-list/?page=${page}&limit=10`;
      ...
      >
        <div className="mb-6 flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">All patient appointments in the system.</p>
          </div>
                ...
                {loading ? (
                  ...
                  appointments.map((appointment) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-4">
                        <span className={`inline-flex items-center gap-1 rounded-full px-3 py-1.5 text-xs font-medium ring-1 ${getStatusColor(appointment.status)}`}>
                          <span className="h-1.5 w-1.5 rounded-full bg-current" />
                          {appointment.status || "N/A"}
                        </span>
                      </td>
