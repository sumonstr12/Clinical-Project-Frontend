...
const steps = [
  { num: 1, icon: "fas fa-calendar-alt", title: "Select Date & Time", desc: "Pick a convenient date from the calendar and choose an available time slot that fits your schedule." },
  { num: 2, icon: "fas fa-user-md", title: "Choose Your Doctor", desc: "Browse our specialists by specialty, rating, or availability. Read patient reviews to find your perfect match." },
        ...
        <div className="appointment-container">
              ...
              {steps.map((step) => (
                <div key={step.num} className="step-item" role="listitem">
                  ...
                  <div className="step-content">
                    <h4>
                      <i className={step.icon} aria-hidden="true" style={{ marginRight: "0.5rem", color: "var(--teal)" }} />
                      {step.title}
                    </h4>
                    <p>{step.desc}</p>
                  </div>
          ...
          <div className="calendar-widget" role="region" aria-label="Appointment Calendar">
            <div className="cal-header">
              <h4>
                <i className="fas fa-calendar-alt" aria-hidden="true" style={{ color: "var(--teal)", marginRight: "0.5rem" }} />
                {MONTHS[month]} {year}
              </h4>
            ...
            <div className="cal-days" role="grid">
              {calDates.map((d, i) => {
                if (!d) return <div key={`empty-${i}`} className="cal-date empty" role="gridcell" />;
                const past = isPast(d);
                return (
                  <div
                    key={d}
                    className={`cal-date${selectedDate === d ? " selected" : ""}${isToday(d) && selectedDate !== d ? " today" : ""}${past ? " inactive" : ""}`}
                    onClick={() => { if (!past) { setSelectedDate(d); setSelectedSlot(null); } }}
                    role="gridcell"
            ...
            <div className="time-slots">
              <h5>
                <i className="fas fa-clock" aria-hidden="true" style={{ color: "var(--teal)", marginRight: "0.4rem" }} />
              ...
              <div className="slots-grid" role="group" aria-label="Available time slots">
                {TIME_SLOTS.map((s) => {
                  ...
                  return (
                    <button
                      key={s.time}
                      className={`slot${pastTime ? " booked" : ""}${selectedSlot === s.time ? " active" : ""}`}
                      disabled={pastTime}
            ...
            >
              <i className="fas fa-user-md" aria-hidden="true" style={{ marginRight: "0.5rem" }} />
              {selectedSlot
                ? `Choose Doctor — ${MONTHS[month].slice(0, 3)} ${selectedDate}, ${selectedSlot}`
